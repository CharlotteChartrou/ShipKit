...
interface StarterProjectMarkProps extends SVGProps<SVGSVGElement> {
  decorative?: boolean;
}

export function StarterProjectMark({
  className,
  decorative = true,
  ...props
}: StarterProjectMarkProps) {
...
interface StarterProjectLogoProps extends HTMLAttributes<HTMLDivElement> {
...
export function StarterProjectLogo({
  className,
  iconClassName,
  locale = "en",
  wordmarkClassName,
  withTagline = false,
  ...props
}: StarterProjectLogoProps) {
  const copy = getLocaleCopy(locale);

  return (
    <div className={cn("flex items-center gap-3", className)} {...props}>
      <StarterProjectMark className={cn("h-10 w-10 shrink-0", iconClassName)} />
